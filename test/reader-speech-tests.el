...
(ert-deftest reader-review-stays-then-advances-only-after-success ()
  (with-temp-buffer
    ...
    (let ((english-reading-mode--active-speech nil)
      ...
      (cl-letf (((symbol-function 'english-reading-mode-speak-current-sentence)
                 (lambda ()
                   (let* ((sentence (reader-document-current-sentence))
                          (context (list :buffer (current-buffer)
                                         :beg (nth 2 sentence) :end (nth 3 sentence))))
                     (push (car sentence) calls)
                     (setq english-reading-mode--active-speech context)
                     (english-reading-mode--review-started context)))))
        (english-reading-mode-read-and-review)
        (should (= (point) 1))
        (english-reading-mode-read-and-review)
        (should (= (length calls) 1))
        ;; Cancellation does not count as having read the sentence.
        ...
        (english-reading-mode--finish english-reading-mode--active-speech)
        (should (= (point) 1))
        ;; Looking up a word in this sentence preserves its read state.
        (goto-char 8)
        (english-reading-mode-read-and-review)
        (should (equal (car calls) "Second sentence."))
        (should (= (point) 17))
        ;; Moving back to a different sentence reads it without skipping.
        (goto-char 1)
        (english-reading-mode-read-and-review)
        (should (equal (car calls) "First sentence."))
        (should (= (point) 1))))))

(ert-deftest reader-review-rerender-and-stale-completion ()
  (with-temp-buffer
    (insert "Same text.")
    (let* ((old (list :buffer (current-buffer) :beg 1 :end (point-max)))
           (new (copy-sequence old)))
      (english-reading-mode--review-started old)
      (let ((key (plist-get english-reading-mode--review-state :key)))
        (erase-buffer)
        (insert "Same text.")
        (should-not (equal key (english-reading-mode--review-key 1 (point-max)))))
      (english-reading-mode--review-started new)
      (nconc old (list :completed t))
      (english-reading-mode--review-finished old)
      (should-not (plist-get english-reading-mode--review-state :read)))))

(ert-deftest reader-review-player-failure-is-not-read ()
  (dolist (success '(nil t))
    (with-temp-buffer
      (insert "A sentence.")
      (let* ((context (list :buffer (current-buffer) :beg 1 :end (point-max)))
             (english-reading-mode--active-speech context)
             (english-reading-mode-speech-finish-hook
              '(english-reading-mode--review-finished))
             (kokoro-reader--request-process nil)
             (kokoro-reader--player-process nil)
             (kokoro-reader--playback-succeeded-p success))
        (english-reading-mode--review-started context)
        (english-reading-mode--player-finished)
        (should (eq (plist-get english-reading-mode--review-state :read) success))))))

(ert-deftest reader-review-pdf-uses-hidden-sentence-state ()
  (let ((speech-buffer (generate-new-buffer " *review-pdf-text*"))
        ...
        (with-temp-buffer
          (let ((source (current-buffer)))
            (with-current-buffer speech-buffer
              (insert "PDF sentence.")
              (let ((context (list :buffer speech-buffer :beg 1 :end (point-max)
                                   :completed t)))
                (english-reading-mode--review-started context)
                (english-reading-mode--review-finished context)))
            (cl-letf (((symbol-function 'reader-document-current-sentence)
                       (lambda () (list "PDF sentence." speech-buffer 1 14)))
                      ((symbol-function 'reader-document-call)
                       (lambda (operation &rest _)
                         (should (eq (current-buffer) source))
                         (should (eq operation :continue))
                         (setq continued t))))
              (english-reading-mode-read-and-review)
              (should continued))))
      (kill-buffer speech-buffer))))
