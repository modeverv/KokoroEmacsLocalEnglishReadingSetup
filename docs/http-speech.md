...
これで通常の `s` 等による読み上げも、先読みを含めてHTTPサーバーへ送られます。
有効化時は再生を一度停止するので、現在の位置から `s` で再開してください。
言語・音声・速度は各読書バッファの既存設定を引き継ぎます。
Emacs側の常駐ネイティブプレイヤーは、サーバーからダウンロードしたWAVの再生だけを担当します。
複数の先読み枠を予約してWAVを順次ロードするため、文完了通知、PDFハイライト、
EPUB・Kindleのページ送りは既存の処理を使えます。端末内での直接合成は行いません。
この経路は読書チャンクごとに受信を完了してから常駐プレイヤーへ渡します。

`M-x reader-http-speech-transport-mode` で解除できます。再起動後も利用するには、
このブランチを使うEmacsの設定で上の2行を読み込んでください。サーバーはGUIから起動してください。
...
独立した追加コマンドはテキストをまとめて送る方式です。EPUBは現在の章が対象で、
...
追加テストはPython 13件、ERT 5件が成功しました。既存の `make my-read-k-ert` は
