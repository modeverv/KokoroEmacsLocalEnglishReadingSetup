...
- `j` で現在文を読み、読み上げ済みなら次文へ進んで読む。`k` で前の1文へ移動
...
3. 本文では`j`で1文ずつ読み進め、`k`で前の文へ移動し、`SPC`で再読、`s`で現在のWebページを連続読み上げします。
...
移動します。`j`は現在文を読み、読み上げ済みなら次文へ進んで読みます。
`k`は前の文へ移動し、ページ境界では表示ページも
...
| キー | 動作 |
| --- | --- |
| `j` | 未読なら現在文を読み、読み上げ済みなら次文へ移動して読む。終了後もその文に留まる |
| `k` | 前の1文へ移動 |
| `SPC` | 現在の1文を読み上げ |
...
| `i` | org-noterで現在位置へノートを挿入し、ノート欄へフォーカスを移す |
| `C-c b` | my-read内のどのペインからでも中央本文へ戻る |
...
読み上げ・単語登録のキーは、my-readフレームの読書ペインにカーソルがある場合だけ
有効です。`i`でノートを作成した後は、そのままノート欄で入力できます。
`C-c b`はmy-read内の全ペインで有効で、本文へ戻って`j`で読み進められます。
通常のEmacsフレームのキー設定には影響しません。org-noterの
